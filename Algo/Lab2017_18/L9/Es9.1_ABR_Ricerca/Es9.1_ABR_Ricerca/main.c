...
void deepthNode(Node * t,int k,int h){
    if(t == NULL){printf("NO\n");return;}
    if(t->key == k){printf("%d\n",h);return;}
    if(k < t->key)deepthNode(t->left,k,h+1);//Search left
    else deepthNode(t->right,k,h+1);//Search right
}
...
int main(int argc, const char * argv[]) {
    ...
    tree = readABR(&n);
    //printABR_Intermediate(tree); //DEBUG
    if(n<=0)return -1;
    //printf("\n");
    do{
        scanf("%d",&k);
        if(k != -1) deepthNode(tree,k,0);
    }while(k >= 0);
