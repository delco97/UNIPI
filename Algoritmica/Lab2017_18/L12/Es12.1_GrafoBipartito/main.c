...
int BFS(Graph g,int s){
    ...
    q.n = 0;
	 //Init source node
    g.a[s].c = B; //could be R, logic wold not change
    g.a[s].prev = -1;
    g.a[s].d = 0;
    tailInsert(&q,s);
    while(q.n != 0 && isBipartite == 1){
        ...
        while(cur != NULL){//Loop adj nodes of u node
            if(g.a[cur->k].c == W){//White adj node
                g.a[cur->k].c = g.a[u].c == B ? R:B;
                g.a[cur->k].d = g.a[u].d + 1;
                g.a[cur->k].prev = u;
                tailInsert(&q,cur->k);
            }else{
            	if(g.a[cur->k].c == g.a[u].c) isBipartite = 0;
            }
            cur = cur->next;
        }
    }
...
int isBipartite(Graph g){
    ...
    for(int i=0;i<g.n;i++){
    	if(g.a[i].c == W){
    		if(BFS(g,i) == 0) return 0; //g is not bipartite
    	}
    }
    return 1;
}
